...
describe("Reading Lists API", () => {
  it("can add a blog to reading list", async () => {
    ...
    assert.strictEqual(response.data.blogUserId, testData.users[0].id);
  ...
  it("returns 404 when blogId is missing", async () => {
    const readingListEntry = {};
  ...
  it("user can view their reading list", async () => {
    ...
    assert.ok(reading.readinglists.length > 0);
    assert.strictEqual(typeof reading.readinglists[0].read, "boolean");
  });
  ...
  it("user can mark a blog as read with authentication", async () => {
    ...
    const readingListId = userResponse.data.readings[0].readinglists[0].id;
...
describe("Session Management API", () => {
  ...
  it("logout removes the active session", async () => {
  ...
  it("multiple logins reuse the existing session", async () => {
    ...
    const response = await axios.post(
      `${baseUrl}/blogs`,
      {
        title: "Blog with Reused Session",
        author: "Reused Token",
        url: "https://example.com/reused-token",
      },
      {
  ...
  it("logout removes the current session for that user", async () => {
    ...
    const newBlog = {
      title: "Blog After Session Removal",
      author: "Session Removed",
      url: "https://example.com/session-removed",
    };
